      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid w-full items-center gap-1.5">
          ...
          <Input
            ...
            value={task.title}
            onChange={async (e) => await updateTask({ ...task, title: e.target.value })}
          />
        </div>
        ...
        <div className="grid w-full items-center gap-1.5">
          ...
          <Calendar
            value={date}
            onChange={async (newDate) => await setDate(newDate)}
          />
        </div>
            ...
            <SelectContent>
              {labels.statuses.map((status) => (
                <SelectItem
                  key={status.value}
                  value={status.value}
                  onClick={async () => await updateTask({ ...task, status: status.value })}
                >
                  {status.label}
                </SelectItem>
              ))}
        ...
        <div className="grid w-full items-center gap-1.5">
          ...
          <Checkbox
            ...
            onChange={async (e) => await updateTask({ ...task, completed: e.target.checked })}
          />
        </div>
        ...
        <div className="grid w-full items-center gap-1.5">
          ...
          <Textarea
            ...
            onChange={async (e) => await updateTask({ ...task, description: e.target.value })}
          />
        </div>
